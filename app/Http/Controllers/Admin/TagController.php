...
class TagController extends Controller
{
    ...
    public function posttagAdd(TagAddRequest $request){
      ...
      if ($request->btnSaveNew) {
            return redirect()->route('them-tag')->with('success','Bạn đã thêm danh mục thành công');
        } else {
            return redirect()->route('danh-sach-tag')->with('success','Bạn đã thêm danh mục thành công');
        } 
    }
    ...
    public function posttagEdit(TagEditRequest $request, $id){
      ...
      $cate->save();
      if ($request->btnSaveNew) {
            return redirect()->route('gettagEdit',['id' => $id])->with('success','Bạn đã thêm danh mục thành công');
        } else {
            return redirect()->route('danh-sach-tag')->with('success','Bạn đã thêm danh mục thành công');
        }  
    }
    public function gettagDel($id){
                $cate = tag::find($id);
                $cate ->delete();
                 return redirect()->route('danh-sach-tag')->with('success', 'Xóa thành công');
    }
  }
