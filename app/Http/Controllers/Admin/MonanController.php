...
class MonanController extends Controller
{
     ...
     public function postMonanAdd(MonanAddRequest $request)
    {
        ...
        if ($request->btnSaveNew) {
            return redirect()->route('them-mon-an')->with('success','Bạn đã thêm thành công');
        } else {
            return redirect()->route('danh-sach-mon-an')->with('success','Bạn đã thêm thành công');
        }
    ...
    public function getMonanDel($id){
        ...
        $news->delete();
         return redirect()->route('danh-sach-mon-an')->with('success', 'Xóa thành công');
    }
     ...
     public function postMonanEdit(MonanEditRequest $request,$id){
        ...
        $news->updated_at =new DateTime();
         if ($request->btnSaveNew) {
            return redirect()->route('them-mon-an')->with('success','Bạn đã sửa thành công');
        } else {
            return redirect()->route('danh-sach-mon-an')->with('success','Bạn đã sửa thành công');
        }
    }
