...
class DMdanhgiaController extends Controller
{
    ...
    public function postDMdanhgiaAdd(DMdanhgiaAddRequest $request){
      ...
      if ($request->btnSaveNew) {
            return redirect()->route('them-danh-muc-danh-gia')->with('success','Bạn đã thêm danh mục thành công');
        } else {
            return redirect()->route('danh-sach-danh-muc-danh-gia')->with('success','Bạn đã thêm danh mục thành công');
        } 
    }
    ...
    public function postdanhgiaEdit(DMdanhgiaEditRequest $request, $id){
      ...
      $cate->save();
      if ($request->btnSaveNew) {
            return redirect()->route('getDMdanhgiaEdit',['id' => $id])->with('success','Bạn đã thêm danh mục thành công');
        } else {
            return redirect()->route('danh-sach-danh-muc-danh-gia')->with('success','Bạn đã thêm danh mục thành công');
        }  
    }
    public function getDMdanhgiaDel($id){
                $cate = DMDanhgia::find($id);
                $cate ->delete();
                 return redirect()->route('danh-sach-danh-muc-danh-gia')->with('success', 'Xóa thành công');
    }
  }
