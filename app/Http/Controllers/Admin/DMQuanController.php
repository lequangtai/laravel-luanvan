...
class DMQuanController extends Controller
{
    ...
    public function postDMquanAdd(DMQuanAddRequest $request){
      ...
      if ($request->btnSaveNew) {
            return redirect()->route('them-quan')->with('success','Bạn đã thêm danh mục thành công');
        } else {
            return redirect()->route('danh-sach-quan')->with('success','Bạn đã thêm danh mục thành công');
        } 
    }
    ...
    public function postquanEdit(DMQuanEditRequest $request, $id){
      ...
      $cate->save();
      if ($request->btnSaveNew) {
            return redirect()->route('getDMquanEdit',['id' => $id])->with('success','Bạn đã thêm danh mục thành công');
        } else {
            return redirect()->route('danh-sach-quan')->with('success','Bạn đã thêm danh mục thành công');
        }  
    }
    public function getDMquanDel($id){
                $cate = DMQuan::find($id);
                $cate ->delete();
                 return redirect()->route('danh-sach-quan')->with('success', 'Xóa thành công');
    }
  }
