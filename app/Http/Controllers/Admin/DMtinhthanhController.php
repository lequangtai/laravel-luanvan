...
class DMtinhthanhController extends Controller
{
    ...
    public function postDMtinhAdd(DMTinhAddRequest $request){
      ...
      if ($request->btnSaveNew) {
            return redirect()->route('them-tinh-thanh')->with('success','Bạn đã thêm danh mục thành công');
        } else {
            return redirect()->route('danh-sach-tinh-thanh')->with('success','Bạn đã thêm danh mục thành công');
        } 
    }
    ...
    public function postCateEdit(DMTinhEditRequest $request, $id){
      ...
      $cate->save();
      if ($request->btnSaveNew) {
            return redirect()->route('getDMtinhEdit',['id' => $id])->with('success','Bạn đã thêm danh mục thành công');
        } else {
            return redirect()->route('danh-sach-tinh-thanh')->with('success','Bạn đã thêm danh mục thành công');
        }  
    }
    public function getDMtinhDel($id){
                $cate = DMTinh::find($id);
                $cate ->delete();
                 return redirect()->route('danh-sach-tinh-thanh')->with('success', 'Xóa thành công');
    }
  }
