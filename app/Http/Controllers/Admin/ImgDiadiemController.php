...
class ImgDiadiemController extends Controller
{
     ...
     public function postImgDiadiemAdd(ImgDiadiemAddRequest $request)
    {
        ...
        if ($request->btnSaveNew) {
            return redirect()->route('them-hinh-anh-dia-diem')->with('success','Bạn đã thêm thành công');
        } else {
            return redirect()->route('danh-sach-hinh-anh-dia-diem')->with('success','Bạn đã thêm thành công');
        }
    ...
    public function getImgDiadiemDel($id){
        ...
        $news->delete();
         return redirect()->route('danh-sach-hinh-anh-dia-diem')->with('success', 'Xóa thành công');
    }
     ...
     public function postImgDiadiemEdit(ImgDiadiemEditRequest $request,$id){
        ...
        $news->updated_at =new DateTime();
         if ($request->btnSaveNew) {
            return redirect()->route('them-hinh-anh-dia-diem')->with('success','Bạn đã sửa thành công');
        } else {
            return redirect()->route('danh-sach-hinh-anh-dia-diem')->with('success','Bạn đã sửa thành công');
        }
    }
