...
class ImgMonanController extends Controller
{
     ...
     public function postImgMonanAdd(ImgMonanAddRequest $request)
    {
        ...
        if ($request->btnSaveNew) {
            return redirect()->route('them-hinh-anh-mon-an')->with('success','Bạn đã thêm thành công');
        } else {
            return redirect()->route('danh-sach-hinh-anh-mon-an')->with('success','Bạn đã thêm thành công');
        }
    ...
    public function getImgMonanDel($id){
        ...
        $news->delete();
         return redirect()->route('danh-sach-hinh-anh-mon-an')->with('success', 'Xóa thành công');
    }
     ...
     public function postImgMonanEdit(ImgMonanEditRequest $request,$id){
        ...
        $news->updated_at =new DateTime();
         if ($request->btnSaveNew) {
            return redirect()->route('them-hinh-anh-mon-an')->with('success','Bạn đã sửa thành công');
        } else {
            return redirect()->route('danh-sach-hinh-anh-mon-an')->with('success','Bạn đã sửa thành công');
        }
    }
