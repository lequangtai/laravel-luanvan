...
class DMmonanController extends Controller
{
    ...
    public function postDMmonanAdd(DMMonanAddRequest $request){
      ...
      if ($request->btnSaveNew) {
            return redirect()->route('them-danh-muc-mon-an')->with('success','Bạn đã thêm danh mục thành công');
        } else {
            return redirect()->route('danh-sach-danh-muc-mon-an')->with('success','Bạn đã thêm danh mục thành công');
        } 
    }
    ...
    public function postmonanEdit(DMMonanEditRequest $request, $id){
      ...
      $cate->save();
      if ($request->btnSaveNew) {
            return redirect()->route('getDMmonanEdit',['id' => $id])->with('success','Bạn đã thêm danh mục thành công');
        } else {
            return redirect()->route('danh-sach-danh-muc-mon-an')->with('success','Bạn đã thêm danh mục thành công');
        }  
    }
    public function getDMmonanDel($id){
                $cate = DMMonan::find($id);
                $cate ->delete();
                 return redirect()->route('danh-sach-danh-muc-mon-an')->with('success', 'Xóa thành công');
    }
  }
