...
class DiadiemController extends Controller
{
    public function getdiadiemAdd()
    {
       $diadiem = Diadiem::select()->get()->toArray();
        return view('User.page.link.location',['diadiem'=>$diadiem]); 
     ...
     public function postdiadiemAdd(DiadiemAddRequest $request)
    {
        ...
        if ($request->btnSaveNew) {
            return redirect()->route('them-dia-diem')->with('success','Bạn đã thêm thành công');
        } else {
            return redirect()->route('danh-sach-dia-diem')->with('success','Bạn đã thêm thành công');
        }
